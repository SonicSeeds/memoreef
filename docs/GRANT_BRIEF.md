...
MemoReef turns saved sources into local Markdown Drops. A Drop is a small, readable note representing a saved source. It includes core fields such as title, URL, type, status, agent readiness, treasure state, folders, tags, import source, and other frontmatter designed to work well in Obsidian and with future agent workflows.
...
MemoReef should not let agents blindly summarize or act on a giant messy archive. The Drift state is the inbox for imported Drops. Review Mode is the human triage layer where the user decides what matters before agents dive in.

The triage model is intentionally simple: keep useful sources in the Reef, mark high-value sources as Treasures, send long-term or uncertain material to the Deep, and discard weak material. This step protects the quality of future agent output. It also gives the user a fast way to convert old saved links into a curated library that reflects current priorities rather than historical clutter.

The repo already includes triage-ready frontmatter fields, a browser-only Review Mode fallback, and a local filesystem-backed Review Mode server. The local app loads Drift Drops from an Obsidian-compatible vault, lets the user sort with keyboard or button controls, autosaves Keep/Treasure/Sink decisions back to Markdown frontmatter, archives sunk Drops outside the active reef with a 30-day delete marker, and can tag kept or Treasured Drops locally after review. This turns human triage into immediate agent-readable structure without requiring a hosted account or external AI service.
...
Currently implemented: a Python CLI package, browser bookmark HTML import, plain URL list import, CSV link import, best-effort web article extraction into Markdown Drops, Obsidian-compatible Markdown Drops, Drift/Reef/Deep/Discarded/Treasure frontmatter fields, import inspection, import logs, a guided offline pilot command and checklist page, local Review Mode JSON export/application, filesystem-backed Review Mode autosave, phone triage over trusted LAN/Tailscale with a self-serve URL/QR workflow, localhost bookmarklet clipping, Sink-to-Discarded archive lifecycle with 30-day delete markers, local reviewed-Drop tagging for kept or Treasured items, agent finish plans and deterministic proposals, duplicate/link/metadata/garden reports, library search, Pearl Dive retrieval with generated Dive Reports and a local app page, project brief export, a demo vault, a generated local dashboard/pilot/library/Pearl Dive/tour with Drop detail, review launcher, reports, and briefs pages, a static landing page, tester guides, GitHub issue templates, and browser-only triage prototypes.

Not implemented yet: LLM-generated summaries and deeper semantic tagging, an Obsidian plugin, a browser extension, a mobile app with vault sync, or a hosted sync/account layer. Web article extraction is present but best-effort; paywalls, blocked pages, JavaScript-only pages, and unreadable HTML are recorded as extraction failures.

## Next Milestones

The next milestones are to collect 2–3 trusted tester signals, improve the quality and reviewability of local tagging/garden suggestions, make article extraction easier to trigger from the first-run flow, add optional local/bring-your-own summarization, refine generated project briefs from curated Drops, and document the storage conventions so other local-first tools can read and build on MemoReef vaults.
