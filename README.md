...
For phone triage on a trusted LAN or Tailscale network, start the same local app in mobile/LAN mode on the Mac that has the vault:

```bash
python3.11 -m memoreef.cli serve --vault /tmp/memoreef-pilot --mobile
```

The command binds to `0.0.0.0`, prints phone-friendly URLs such as `http://192.168.x.x:8765/` or `http://100.x.x.x:8765/`, and warns that the vault write API is reachable from that network. Open one of those URLs on the phone while it is on the same trusted LAN/Tailscale. Decisions still write to the local vault through the Mac server.
...
Phone/LAN/Tailscale mode uses the same real Review Mode and the same local Markdown writes:

```bash
python3.11 -m memoreef.cli serve --vault /tmp/memoreef-pilot --limit 25 --lan
```

Keep the server running on the Mac, put the phone on the same trusted LAN or Tailscale network, then open one of the printed `http://<mac-ip>:8765/` URLs on the phone. This is not a hosted sync layer or mobile app: the phone talks to the Mac, and the Mac writes Keep/Pearl/Sink decisions to the local vault.
