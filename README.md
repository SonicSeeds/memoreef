...
| Review Mode prototype: loads real review-session JSON | Mobile app mockup: visual only |
| --- | --- |
...
The Review Mode prototype opens with clearly marked sample data. To review your real bookmarks, load one of the JSON files generated by the pilot command:
...
The separate mobile app mockup at `site/mobile.html` is visual only. It does not connect to an Obsidian vault on your computer. A working mobile version would need vault sync, a local companion server, or a packaged mobile app with file-provider access.
